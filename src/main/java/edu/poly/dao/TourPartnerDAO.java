...
public class TourPartnerDAO extends JdbcDaoSupport {
    ...
    public List<TourPartnerDTO> getTourByIdDTO(Integer id) {
        String sql = "SELECT t.id, t.Name, t.Address, t.Content, t.Image, t.Policy, t.Lat, t.Lng, s.id as 'ServiceId', s.Name as 'ServiceName', s.Price, s.Normal_price, s.Detail, s.Rule, s.Is_default, s.Is_deleted FROM tours t LEFT JOIN services s ON s.tour_id = t.id WHERE t.id = ?";
