...
public class OrderDAO  extends JdbcDaoSupport {
    ...
    public List<OrderDTO> getListOrderDTO() {
        String sql = "SELECT orders.id, users.`Name` AS customername, services.`Name` AS servicename, users.Email AS email, orders.Payment AS payment, orders.Is_purchased AS purchased, orders.Created_at AS datecreate, orders.`Data` AS `data`, tours.Address AS address, tours.`Name` AS tourname, tours.Policy AS policy, orders.Is_deleted as deleted,orders.user_id as userid ,tours.user_id as partnerid FROM orders INNER JOIN users ON orders.user_id = users.id INNER JOIN services ON orders.service_id = services.id INNER JOIN tours ON services.tour_id = tours.id WHERE orders.Payment = 1 or orders.Payment = 2";
    ...
    public List<OrderDTO> getListOrderCustomerDTO(Integer id) {
        String sql = "SELECT orders.id, users.`Name` AS customername, services.`Name` AS servicename, users.Email AS email, orders.Payment AS payment, orders.Is_purchased AS purchased, orders.Created_at AS datecreate, orders.`Data` AS `data`, tours.Address AS address, tours.`Name` AS tourname, tours.Policy AS policy, orders.Is_deleted as deleted,orders.user_id as userid ,tours.user_id as partnerid FROM orders INNER JOIN users ON orders.user_id = users.id INNER JOIN services ON orders.service_id = services.id INNER JOIN tours ON services.tour_id = tours.id WHERE users.id = ? AND orders.Is_deleted = false AND orders.Is_purchased = false";
        Object[] params = new Object[]{id};
        OrderMapper mapper = new OrderMapper();
        List<OrderDTO> list =  this.getJdbcTemplate().query(sql, params, mapper);
        return list;
    ...
    public List<OrderDTO> getListOrderPartnerDTO(Integer id) {
        String sql = "SELECT orders.id, users.`Name` AS customername, services.`Name` AS servicename, users.Email AS email, orders.Payment AS payment, orders.Is_purchased AS purchased, orders.Created_at AS datecreate, orders.`Data` AS `data`, tours.Address AS address, tours.`Name` AS tourname, tours.Policy AS policy, orders.Is_deleted as deleted,orders.user_id as userid ,tours.user_id as partnerid FROM orders INNER JOIN users ON orders.user_id = users.id INNER JOIN services ON orders.service_id = services.id INNER JOIN tours ON services.tour_id = tours.id WHERE orders.Payment = 3 and orders.Is_deleted = false and orders.Is_purchased = false and tours.user_id = ?";
        Object[] params = new Object[]{id};
        OrderMapper mapper = new OrderMapper();
        List<OrderDTO> list =  this.getJdbcTemplate().query(sql, params, mapper);
        return list;
    ...
    public List<OrderDTO> getListPurchaseOrderPartnerDTO(Integer id) {
        String sql = "SELECT orders.id, users.`Name` AS customername, services.`Name` AS servicename, users.Email AS email, orders.Payment AS payment, orders.Is_purchased AS purchased, orders.Created_at AS datecreate, orders.`Data` AS `data`, tours.Address AS address, tours.`Name` AS tourname, tours.Policy AS policy, orders.Is_deleted as deleted,orders.user_id as userid ,tours.user_id as partnerid FROM orders INNER JOIN users ON orders.user_id = users.id INNER JOIN services ON orders.service_id = services.id INNER JOIN tours ON services.tour_id = tours.id WHERE orders.Payment = 3 and orders.Is_deleted = false and orders.Is_purchased = true and tours.user_id = ?";
        Object[] params = new Object[]{id};
        OrderMapper mapper = new OrderMapper();
        List<OrderDTO> list =  this.getJdbcTemplate().query(sql, params, mapper);
        return list;
    ...
    public Integer deleteOrderAdmin(){
        String sql = "UPDATE orders INNER JOIN users ON orders.user_id = users.id INNER JOIN services ON orders.service_id = services.id INNER JOIN tours ON services.tour_id = tours.id SET orders.Is_deleted = true,orders.Updated_at = CONVERT_TZ(NOW(), @@session.time_zone, '+7:00') WHERE (UNIX_TIMESTAMP(CURRENT_TIMESTAMP)*1000) - (UNIX_TIMESTAMP(orders.Created_at)*1000) > (tours.Policy * 3600000) and orders.Is_purchased = false";
        Integer ok =  this.getJdbcTemplate().update(sql);
        return ok;
    }
    ...
    public Integer updatePurchaseOrder(Boolean purchased,Integer id){
        String sql = "UPDATE orders SET orders.Is_purchased = "+purchased+", orders.Updated_at = CONVERT_TZ(NOW(), @@session.time_zone, '+7:00') WHERE orders.id = "+id+"";
        Integer ok =  this.getJdbcTemplate().update(sql);
        return ok;
    }
